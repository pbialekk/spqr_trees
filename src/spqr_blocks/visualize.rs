use crate::{
    ...
    triconnected_blocks::{outside_structures::ComponentType, visualize::visualize_triconnected},
};
    ...
    for (u, adj_u) in spqr.adj.iter().enumerate() {
        for &v in adj_u {
            if u < v {
                ...
                let u_prefix = match u_type.clone().unwrap() {
                    ComponentType::R => "R",
                    ComponentType::P => "P",
                    ComponentType::S => "S",
                };
                let v_prefix = match v_type.clone().unwrap() {
                    ComponentType::R => "R",
                    ComponentType::P => "P",
                    ComponentType::S => "S",
                };
